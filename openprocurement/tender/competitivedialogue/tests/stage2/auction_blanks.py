...
def patch_tender_2lot_auction(self):
    ...
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['errors'][0]["description"], "Can't update auction urls in current ({}) tender status".format(
        self.initial_status if self.initial_status else 'active.pre-qualification.stand-still'
    ))
    ...
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['errors'][0]["description"], [{u'lots': [{u'id': [u'id should be one of lots']}]},
                                                                 {u'lots': [{u'id': [u'id should be one of lots']}]},
                                                                 {u'lots': [{u'id': [u'id should be one of lots']}]}])
