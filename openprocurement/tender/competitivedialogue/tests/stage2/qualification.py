...
class TenderStage2EUQualificationResourceTest(BaseCompetitiveDialogEUStage2ContentWebTest):
    ...
    def test_post_tender_qualifications(self):
        ...
        data = {'bidID': '1234' * 8, 'status': 'pending', 'id': '12345678123456781234567812345678'}
        response = self.app.post_json('/tenders/{}/qualifications/{}'.format(self.tender_id, data['id']),
                                      {'data': data},
                                      status=404)
...
class TenderStage2EU2LotQualificationResourceTest(BaseCompetitiveDialogEUStage2ContentWebTest):
    ...
    def test_patch_tender_qualifications(self):
        ...
        response = self.app.patch_json('/tenders/{}/qualifications/{}?acc_token={}'.format(
            self.tender_id, qualifications[2]['id'], self.tender_token),
            {'data': {'status': 'active', 'qualified': True, 'eligible': True}})
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.json['data']['status'], 'active')
        ...
        response = self.app.patch_json('/tenders/{}/qualifications/{}?acc_token={}'.format(
            self.tender_id, qualifications[0]['id'], self.tender_token),
            {'data': {'status': 'cancelled'}})
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.json['data']['status'], 'cancelled')
    ...
    def test_get_tender_qualifications_collection(self):
        ...
        for bid in response.json['data']:
            response = self.app.get('/tenders/{}/bids/{}?acc_token={}'.format(self.tender_id,
                                                                              bid['id'],
                                                                              self.initial_bids_tokens[bid['id']]))
...
class TenderStage2EUQualificationDocumentResourceTest(BaseCompetitiveDialogEUStage2ContentWebTest):
    ...
    def test_not_found(self):
        response = self.app.post('/tenders/some_id/qualifications/some_id/documents',
                                 status=404,
        ...
        response = self.app.post('/tenders/{}/qualifications/some_id/documents'.format(self.tender_id),
                                 status=404,
                                 upload_files=[('file', 'name.doc', 'content')])
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.get('/tenders/some_id/qualifications/some_id/documents?acc_token={}'.format(
            self.tender_token), status=404)
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.get('/tenders/{}/qualifications/some_id/documents?acc_token={}'.format(
        ...
        response = self.app.get('/tenders/some_id/qualifications/some_id/documents/some_id?acc_token={}'.format(
            self.tender_token), status=404)
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        ...
        response = self.app.get('/tenders/{}/qualifications/some_id/documents/some_id?acc_token={}'.format(
            self.tender_id, self.tender_token), status=404)
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        ...
        response = self.app.get('/tenders/{}/qualifications/{}/documents/some_id?acc_token={}'.format(
        ...
        response = self.app.put('/tenders/some_id/qualifications/some_id/documents/some_id?acc_token={}'.format(
            self.tender_token), status=404, upload_files=[('file', 'name.doc', 'content2')])
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
        ...
        response = self.app.put('/tenders/{}/qualifications/some_id/documents/some_id?acc_token={}'.format(
    ...
    def test_create_qualification_document(self):
        ...
        response = self.app.get('/tenders/{}/qualifications/{}/documents?all=true'.format(
...
class TenderStage2EUQualificationComplaintResourceTest(BaseCompetitiveDialogEUStage2ContentWebTest):
    ...
    def test_create_tender_qualification_complaint(self):
        response = self.app.post_json('/tenders/{}/qualifications/{}/complaints?acc_token={}'.format(
            ...
            {'data': {'title': 'complaint title', 'description': 'complaint description',
                      'author': self.bids[0]['tenderers'][0], 'status': 'pending'}})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
    ...
    def test_patch_tender_qualification_complaint(self):
        ...
        response = self.app.patch_json('/tenders/{}/qualifications/{}/complaints/some_id'.format(
        ...
        response = self.app.patch_json('/tenders/some_id/qualifications/some_id/complaints/some_id',
                                       {'data': {'status': 'resolved', 'resolution': 'resolution text'}},
                                       status=404)
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.patch_json('/tenders/{}/qualifications/some_id/complaints/some_id'.format(self.tender_id),
                                       {'data': {'status': 'resolved', 'resolution': 'resolution text'}},
                                       status=404)
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.get('/tenders/{}/qualifications/{}/complaints/{}'.format(self.tender_id,
    ...
    def test_review_tender_qualification_complaint(self):
        for status in ['invalid', 'declined', 'satisfied']:
            ...
            response = self.app.post_json('/tenders/{}/qualifications/{}/complaints?acc_token={}'.format(
                self.tender_id, self.qualification_id, self.initial_bids_tokens.values()[0]),
                {'data': {'title': 'complaint title', 'description': 'complaint description',
                          'author': self.initial_bids[0]['tenderers'][0], 'status': 'pending'}})
            self.assertEqual(response.status, '201 Created')
            self.assertEqual(response.content_type, 'application/json')
            complaint = response.json['data']
    ...
    def test_get_tender_qualification_complaints(self):
        ...
        response = self.app.post_json('/tenders/{}/qualifications/{}/complaints?acc_token={}'.format(
            ...
            {'data': {'title': 'complaint title',
                      'description': 'complaint description',
                      'author': self.bids[0]['tenderers'][0]}}, status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
...
class TenderStage2EULotQualificationComplaintResourceTest(TenderStage2EUQualificationComplaintResourceTest):
    ...
    def test_patch_tender_qualification_complaint(self):
        ...
        response = self.app.patch_json('/tenders/{}/qualifications/{}/complaints/some_id'.format(self.tender_id,
                                                                                                 self.qualification_id),
                                       {'data': {'status': 'resolved', 'resolution': 'resolution text'}},
                                       status=404)
        self.assertEqual(response.status, '404 Not Found')
...
class TenderStage2EUQualificationComplaintDocumentResourceTest(BaseCompetitiveDialogEUStage2ContentWebTest):
    ...
    def test_not_found(self):
        response = self.app.post('/tenders/some_id/qualifications/some_id/complaints/some_id/documents',
                                 status=404,
                                 upload_files=[('file', 'name.doc', 'content')])
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.post('/tenders/{}/qualifications/some_id/complaints/some_id/documents'.format(
            self.tender_id),
            status=404,
            upload_files=[('file', 'name.doc', 'content')])
        self.assertEqual(response.status, '404 Not Found')
        ...
        response = self.app.post('/tenders/{}/qualifications/{}/complaints/some_id/documents'.format(
            self.tender_id, self.qualification_id),
            status=404,
            upload_files=[('file', 'name.doc', 'content')])
        self.assertEqual(response.status, '404 Not Found')
        ...
        response = self.app.get('/tenders/{}/qualifications/some_id/complaints/some_id/documents'.format(
            self.tender_id),
            status=404)
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.get('/tenders/{}/qualifications/{}/complaints/some_id/documents'.format(
            self.tender_id, self.qualification_id),
            status=404)
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.get('/tenders/{}/qualifications/some_id/complaints/some_id/documents/some_id'.format(
            self.tender_id),
            status=404)
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.get('/tenders/{}/qualifications/{}/complaints/some_id/documents/some_id'.format(
            self.tender_id, self.qualification_id),
            status=404)
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.get('/tenders/{}/qualifications/{}/complaints/{}/documents/some_id'.format(
        ...
        response = self.app.put('/tenders/some_id/qualifications/some_id/complaints/some_id/documents/some_id',
                                status=404,
                                upload_files=[('file', 'name.doc', 'content2')])
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.put('/tenders/{}/qualifications/some_id/complaints/some_id/documents/some_id'.format(
            self.tender_id),
            status=404,
            upload_files=[('file', 'name.doc', 'content2')])
        self.assertEqual(response.status, '404 Not Found')
        ...
        response = self.app.put('/tenders/{}/qualifications/{}/complaints/some_id/documents/some_id'.format(
            self.tender_id, self.qualification_id),
            status=404,
            upload_files=[('file', 'name.doc', 'content2')])
        self.assertEqual(response.status, '404 Not Found')
    ...
    def test_create_tender_qualification_complaint_document(self):
        ...
        response = self.app.get('/tenders/{}/qualifications/{}/complaints/{}/documents?all=true'.format(
            self.tender_id, self.qualification_id, self.complaint_id))
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(doc_id, response.json['data'][0]['id'])
    ...
    def test_patch_tender_qualification_complaint_document(self):
        ...
        response = self.app.patch_json('/tenders/{}/qualifications/{}/complaints/{}/documents/{}?acc_token={}'.format(
            self.tender_id, self.qualification_id, self.complaint_id, doc_id, self.tender_token),
            {'data': {'description': 'document description'}},
            status=403)
        self.assertEqual(response.status, '403 Forbidden')
        ...
        response = self.app.put(
            '/tenders/{}/qualifications/{}/complaints/{}/documents/{}?acc_token={}'.format(self.tender_id,
                                                                                           self.qualification_id,
