...
class CompetitiveDialogEUComplaintDocumentResourceTest(BaseCompetitiveDialogEUContentWebTest):
    ...
    def test_not_found(self):
        response = self.app.post('/tenders/some_id/complaints/some_id/documents', status=404,
        ...
        response = self.app.put('/tenders/{}/complaints/some_id/documents/some_id'.format(self.tender_id), status=404,
                                upload_files=[('file', 'name.doc', 'content2')])
        self.assertEqual(response.status, '404 Not Found')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
...
class CompetitiveDialogUAComplaintResourceTest(BaseCompetitiveDialogUAContentWebTest):
    ...
    def test_create_tender_complaint_invalid(self):
        ...
        response = self.app.post_json(request_path,
                                      {'data': {'author': {'identifier': 'invalid_value'}}},
                                      status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
